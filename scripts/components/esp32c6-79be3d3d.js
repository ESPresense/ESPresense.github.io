...
class H extends s {
  constructor() {
    super(...arguments), this.CHIP_NAME = "ESP32-C6", this.IMAGE_CHIP_ID = 13, this.EFUSE_BASE = 1611335680, this.MAC_EFUSE_REG = this.EFUSE_BASE + 68, this.UART_CLKDIV_REG = 1072955412, this.UART_CLKDIV_MASK = 1048575, this.UART_DATE_REG_ADDR = 1610612860, this.FLASH_WRITE_SIZE = 1024, this.BOOTLOADER_FLASH_OFFSET = 0, this.FLASH_SIZES = {
      ...
      "8MB": 48,
      "16MB": 64
    }, this.SPI_REG_BASE = 1610620928, this.SPI_USR_OFFS = 24, this.SPI_USR1_OFFS = 28, this.SPI_USR2_OFFS = 32, this.SPI_MOSI_DLEN_OFFS = 36, this.SPI_MISO_DLEN_OFFS = 40, this.SPI_W0_OFFS = 88, this.TEXT_START = w.text_start, this.ENTRY = w.entry, this.DATA_START = w.data_start, this.ROM_DATA = w.data, this.ROM_TEXT = w.text;
  }
  ...
  async get_chip_revision(E) {
    const A = this.EFUSE_BASE + 68, c = 3, B = 18, t = A + 4 * c;
    return (await E.read_reg(t) & 7 << B) >> B;
  }
  async get_chip_description(E) {
    let A;
    await this.get_pkg_version(E) === 0 ? A = "ESP32-C6" : A = "unknown ESP32-C6";
  ...
  async read_mac(E) {
    ...
    A = A >>> 0;
    let c = await E.read_reg(this.MAC_EFUSE_REG + 4);
    c = c >>> 0 & 65535;
    const B = new Uint8Array(6);
    return B[0] = c >> 8 & 255, B[1] = c & 255, B[2] = A >> 24 & 255, B[3] = A >> 16 & 255, B[4] = A >> 8 & 255, B[5] = A & 255, this._d2h(B[0]) + ":" + this._d2h(B[1]) + ":" + this._d2h(B[2]) + ":" + this._d2h(B[3]) + ":" + this._d2h(B[4]) + ":" + this._d2h(B[5]);
  }
  get_erase_size(E, A) {
    return A;
  }
