import { R as g } from "./rom-yQfzqXdN.js";
...
class T extends g {
  ...
  async getPkgVersion(B) {
    const E = this.EFUSE_BASE + 68 + 4 * 3;
    return await B.readReg(E) >> 21 & 7;
  }
  async getChipRevision(B) {
    const A = this.EFUSE_BASE + 68, w = 3, E = 18, t = A + 4 * w;
    return (await B.readReg(t) & 7 << E) >> E;
  }
  async getChipDescription(B) {
    let A;
    await this.getPkgVersion(B) === 0 ? A = "ESP32-C3" : A = "unknown ESP32-C3";
    const E = await this.getChipRevision(B);
    return A += " (revision " + E + ")", A;
  }
  async getChipFeatures(B) {
    return ["Wi-Fi"];
  }
  async getCrystalFreq(B) {
    return 40;
  }
  _d2h(B) {
  ...
  async readMac(B) {
    let A = await B.readReg(this.MAC_EFUSE_REG);
    A = A >>> 0;
    let w = await B.readReg(this.MAC_EFUSE_REG + 4);
    w = w >>> 0 & 65535;
