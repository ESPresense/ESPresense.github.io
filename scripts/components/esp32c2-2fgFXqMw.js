...
class O extends M {
  ...
  async getPkgVersion(t) {
    const A = this.EFUSE_BASE + 64 + 4 * 1;
    return await t.readReg(A) >> 22 & 7;
  }
  async getChipRevision(t) {
    const E = this.EFUSE_BASE + 64, B = 1, A = 20, s = E + 4 * B;
    return (await t.readReg(s) & 3 << A) >> A;
  }
  async getChipDescription(t) {
    let E;
    const B = await this.getPkgVersion(t);
    B === 0 || B === 1 ? E = "ESP32-C2" : E = "unknown ESP32-C2";
    const A = await this.getChipRevision(t);
    return E += " (revision " + A + ")", E;
  }
  async getChipFeatures(t) {
    return ["Wi-Fi", "BLE"];
  }
  async getCrystalFreq(t) {
    const E = await t.readReg(this.UART_CLKDIV_REG) & this.UART_CLKDIV_MASK, B = t.transport.baudrate * E / 1e6 / this.XTAL_CLK_DIVIDER;
    let A;
    return B > 33 ? A = 40 : A = 26, Math.abs(A - B) > 1 && t.info("WARNING: Unsupported crystal in use"), A;
  }
  ...
  async readMac(t) {
    ...
    E = E >>> 0;
    let B = await t.readReg(this.MAC_EFUSE_REG + 4);
    B = B >>> 0 & 65535;
    const A = new Uint8Array(6);
    return A[0] = B >> 8 & 255, A[1] = B & 255, A[2] = E >> 24 & 255, A[3] = E >> 16 & 255, A[4] = E >> 8 & 255, A[5] = E & 255, this._d2h(A[0]) + ":" + this._d2h(A[1]) + ":" + this._d2h(A[2]) + ":" + this._d2h(A[3]) + ":" + this._d2h(A[4]) + ":" + this._d2h(A[5]);
  }
  getEraseSize(t, E) {
    return E;
  }
