...
class H extends F {
  ...
  async getPkgVersion(E) {
    ...
    return await E.readReg(B) >> 21 & 7;
  }
  async getChipRevision(E) {
    const A = this.EFUSE_BASE + 68, w = 3, B = 18, t = A + 4 * w;
    return (await E.readReg(t) & 7 << B) >> B;
  }
  async getChipDescription(E) {
    let A;
    await this.getPkgVersion(E) === 0 ? A = "ESP32-C6" : A = "unknown ESP32-C6";
    const B = await this.getChipRevision(E);
  ...
  async readMac(E) {
    let A = await E.readReg(this.MAC_EFUSE_REG);
    A = A >>> 0;
    let w = await E.readReg(this.MAC_EFUSE_REG + 4);
    w = w >>> 0 & 65535;
