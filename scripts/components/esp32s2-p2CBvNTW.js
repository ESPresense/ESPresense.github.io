...
class R extends Q {
  ...
  async getPkgVersion(C) {
    ...
    return await C.readReg(g) >> 21 & 15;
  }
  async getChipDescription(C) {
    const A = ["ESP32-S2", "ESP32-S2FH16", "ESP32-S2FH32"], I = await this.getPkgVersion(C);
    return I >= 0 && I <= 2 ? A[I] : "unknown ESP32-S2";
  }
  async getChipFeatures(C) {
    const A = ["Wi-Fi"], I = await this.getPkgVersion(C);
    I == 1 ? A.push("Embedded 2MB Flash") : I == 2 && A.push("Embedded 4MB Flash");
    const g = 4, E = this.EFUSE_BASE + 92 + 4 * g;
    return (await C.readReg(E) >> 4 & 7) == 1 && A.push("ADC and temperature sensor calibration in BLK2 of efuse"), A;
  }
  ...
  async readMac(C) {
    let A = await C.readReg(this.MAC_EFUSE_REG);
    A = A >>> 0;
    let I = await C.readReg(this.MAC_EFUSE_REG + 4);
    I = I >>> 0 & 65535;
