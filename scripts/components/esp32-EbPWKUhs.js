...
class M extends o {
  constructor() {
    super(...arguments), this.CHIP_NAME = "ESP32", this.IMAGE_CHIP_ID = 0, this.EFUSE_RD_REG_BASE = 1073061888, this.DR_REG_SYSCON_BASE = 1073111040, this.UART_CLKDIV_REG = 1072955412, this.UART_CLKDIV_MASK = 1048575, this.UART_DATE_REG_ADDR = 1610612856, this.XTAL_CLK_DIVIDER = 1, this.FLASH_SIZES = {
      ...
      "8MB": 48,
      "16MB": 64
    }, this.FLASH_WRITE_SIZE = 1024, this.BOOTLOADER_FLASH_OFFSET = 4096, this.SPI_REG_BASE = 1072963584, this.SPI_USR_OFFS = 28, this.SPI_USR1_OFFS = 32, this.SPI_USR2_OFFS = 36, this.SPI_W0_OFFS = 128, this.SPI_MOSI_DLEN_OFFS = 40, this.SPI_MISO_DLEN_OFFS = 44, this.TEXT_START = B.text_start, this.ENTRY = B.entry, this.DATA_START = B.data_start, this.ROM_DATA = B.data, this.ROM_TEXT = B.text;
  }
  async readEfuse(C, A) {
    const g = this.EFUSE_RD_REG_BASE + 4 * A;
    return C.debug("Read efuse " + g), await C.readReg(g);
  }
  async getPkgVersion(C) {
    const A = await this.readEfuse(C, 3);
    let g = A >> 9 & 7;
    return g += (A >> 2 & 1) << 3, g;
  }
  async getChipRevision(C) {
    const A = await this.readEfuse(C, 3), g = await this.readEfuse(C, 5), I = await C.readReg(this.DR_REG_SYSCON_BASE + 124), t = A >> 15 & 1, i = g >> 20 & 1, s = I >> 31 & 1;
    return t != 0 ? i != 0 ? s != 0 ? 3 : 2 : 1 : 0;
  }
  async getChipDescription(C) {
    const A = [
      "ESP32-D0WDQ6",
      "ESP32-D0WD",
      "ESP32-D2WD",
    ...
    let g = "";
    const I = await this.getPkgVersion(C), t = await this.getChipRevision(C), i = t == 3;
    return (await this.readEfuse(C, 3) & 1) != 0 && (A[0] = "ESP32-S0WDQ6", A[1] = "ESP32-S0WD"), i && (A[5] = "ESP32-PICO-V3"), I >= 0 && I <= 6 ? g = A[I] : g = "Unknown ESP32", i && (I === 0 || I === 1) && (g += "-V3"), g + " (revision " + t + ")";
  }
  async getChipFeatures(C) {
    const A = ["Wi-Fi"], g = await this.readEfuse(C, 3);
    (g & 2) === 0 && A.push(" BT"), (g & 1) !== 0 ? A.push(" Single Core") : A.push(" Dual Core"), (g & 8192) !== 0 && ((g & 4096) !== 0 ? A.push(" 160MHz") : A.push(" 240MHz"));
    const s = await this.getPkgVersion(C);
    [2, 4, 5, 6].indexOf(s) !== -1 && A.push(" Embedded Flash"), s === 6 && A.push(" Embedded PSRAM"), (await this.readEfuse(C, 4) >> 8 & 31) !== 0 && A.push(" VRef calibration in efuse"), (g >> 14 & 1) !== 0 && A.push(" BLK3 partially reserved");
    const E = await this.readEfuse(C, 6) & 3, e = ["None", "3/4", "Repeat (UNSUPPORTED)", "Invalid"];
    return A.push(" Coding Scheme " + e[E]), A;
  }
  async getCrystalFreq(C) {
    const A = await C.readReg(this.UART_CLKDIV_REG) & this.UART_CLKDIV_MASK, g = C.transport.baudrate * A / 1e6 / this.XTAL_CLK_DIVIDER;
    let I;
  ...
  async readMac(C) {
    let A = await this.readEfuse(C, 1);
    A = A >>> 0;
    let g = await this.readEfuse(C, 2);
    g = g >>> 0;
