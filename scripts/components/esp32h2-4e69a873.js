...
class H extends g {
  ...
  async _post_connect(B) {
    const A = await B.read_reg(this.UARTDEV_BUF_NO) & 255;
    B.debug("In _post_connect " + A), A == this.UARTDEV_BUF_NO_USB && (B.ESP_RAM_BLOCK = this.USB_RAM_BLOCK);
  }
  async read_mac(B) {
    let A = await B.read_reg(this.MAC_EFUSE_REG);
    A = A >>> 0;
    let c = await B.read_reg(this.MAC_EFUSE_REG + 4);
    c = c >>> 0 & 65535;
    const E = new Uint8Array(6);
    return E[0] = c >> 8 & 255, E[1] = c & 255, E[2] = A >> 24 & 255, E[3] = A >> 16 & 255, E[4] = A >> 8 & 255, E[5] = A & 255, this._d2h(E[0]) + ":" + this._d2h(E[1]) + ":" + this._d2h(E[2]) + ":" + this._d2h(E[3]) + ":" + this._d2h(E[4]) + ":" + this._d2h(E[5]);
  }
  get_erase_size(B, A) {
    return A;
  }
