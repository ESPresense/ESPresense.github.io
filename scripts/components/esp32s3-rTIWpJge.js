...
class S extends E {
  constructor() {
    super(...arguments), this.CHIP_NAME = "ESP32-S3", this.IMAGE_CHIP_ID = 9, this.EFUSE_BASE = 1610641408, this.MAC_EFUSE_REG = this.EFUSE_BASE + 68, this.UART_CLKDIV_REG = 1610612756, this.UART_CLKDIV_MASK = 1048575, this.UART_DATE_REG_ADDR = 1610612864, this.FLASH_WRITE_SIZE = 1024, this.BOOTLOADER_FLASH_OFFSET = 0, this.FLASH_SIZES = {
      ...
      "8MB": 48,
      "16MB": 64
    }, this.SPI_REG_BASE = 1610620928, this.SPI_USR_OFFS = 24, this.SPI_USR1_OFFS = 28, this.SPI_USR2_OFFS = 32, this.SPI_MOSI_DLEN_OFFS = 36, this.SPI_MISO_DLEN_OFFS = 40, this.SPI_W0_OFFS = 88, this.USB_RAM_BLOCK = 2048, this.UARTDEV_BUF_NO_USB = 3, this.UARTDEV_BUF_NO = 1070526796, this.TEXT_START = I.text_start, this.ENTRY = I.entry, this.DATA_START = I.data_start, this.ROM_DATA = I.data, this.ROM_TEXT = I.text;
  }
  async getChipDescription(g) {
    return "ESP32-S3";
  }
  async getChipFeatures(g) {
    return ["Wi-Fi", "BLE"];
  }
  async getCrystalFreq(g) {
    return 40;
  }
  _d2h(g) {
  ...
  async postConnect(g) {
    const A = await g.readReg(this.UARTDEV_BUF_NO) & 255;
    g.debug("In _post_connect " + A), A == this.UARTDEV_BUF_NO_USB && (g.ESP_RAM_BLOCK = this.USB_RAM_BLOCK);
  }
  async readMac(g) {
    let A = await g.readReg(this.MAC_EFUSE_REG);
    A = A >>> 0;
    let B = await g.readReg(this.MAC_EFUSE_REG + 4);
    B = B >>> 0 & 65535;
    const C = new Uint8Array(6);
    return C[0] = B >> 8 & 255, C[1] = B & 255, C[2] = A >> 24 & 255, C[3] = A >> 16 & 255, C[4] = A >> 8 & 255, C[5] = A & 255, this._d2h(C[0]) + ":" + this._d2h(C[1]) + ":" + this._d2h(C[2]) + ":" + this._d2h(C[3]) + ":" + this._d2h(C[4]) + ":" + this._d2h(C[5]);
  }
  getEraseSize(g, A) {
    return A;
  }
