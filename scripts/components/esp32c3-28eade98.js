...
class e extends g {
  ...
  async get_pkg_version(B) {
    const E = this.EFUSE_BASE + 68 + 4 * 3;
    return await B.read_reg(E) >> 21 & 7;
  }
  async get_chip_revision(B) {
    const A = this.EFUSE_BASE + 68, w = 3, E = 18, t = A + 4 * w;
    return (await B.read_reg(t) & 7 << E) >> E;
  }
  async get_chip_description(B) {
    let A;
    await this.get_pkg_version(B) === 0 ? A = "ESP32-C3" : A = "unknown ESP32-C3";
    const E = await this.get_chip_revision(B);
    return A += " (revision " + E + ")", A;
  }
  async get_chip_features(B) {
    return ["Wi-Fi"];
  }
  async get_crystal_freq(B) {
    return 40;
  }
  _d2h(B) {
  ...
  async read_mac(B) {
    let A = await B.read_reg(this.MAC_EFUSE_REG);
    A = A >>> 0;
    let w = await B.read_reg(this.MAC_EFUSE_REG + 4);
    w = w >>> 0 & 65535;
    const E = new Uint8Array(6);
    return E[0] = w >> 8 & 255, E[1] = w & 255, E[2] = A >> 24 & 255, E[3] = A >> 16 & 255, E[4] = A >> 8 & 255, E[5] = A & 255, this._d2h(E[0]) + ":" + this._d2h(E[1]) + ":" + this._d2h(E[2]) + ":" + this._d2h(E[3]) + ":" + this._d2h(E[4]) + ":" + this._d2h(E[5]);
  }
  get_erase_size(B, A) {
    return A;
  }
